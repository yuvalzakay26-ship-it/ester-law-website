    ...
    return (
        ...
        >
            ...
            <div className="relative mx-auto mt-14 grid max-w-5xl grid-cols-1 gap-6 md:mt-16 md:grid-cols-2 md:gap-8">
                <Reveal delay={150}>
                    <article className="relative h-full overflow-hidden rounded-2xl bg-white p-7 shadow-card ring-1 ring-accent/20 sm:p-8">
                        <span
                            aria-hidden="true"
                ...
                <Reveal delay={250}>
                    <article className="relative h-full overflow-hidden rounded-2xl bg-white p-7 shadow-card ring-1 ring-neutral-200/80 sm:p-8">
                        <span
                            aria-hidden="true"
                            className={`absolute top-0 ${isRTL ? 'right-0' : 'left-0'} h-1 w-20 rounded-b-full bg-neutral-300`}
                        />
                        <h3 className="text-lg font-bold text-primary sm:text-xl">
                            {fit.negativeTitle}
                        </h3>
                        <ul className="mt-6 space-y-4">
                            {fit.negatives.map((line, i) => (
                                <li key={i} className="flex items-start gap-3">
                                    <span className="mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-neutral-100 text-neutral-500 ring-1 ring-neutral-200">
                                        {MinusIcon}
                                    </span>
                                    <span className="text-base leading-relaxed text-neutral-700">
                                        {line}
